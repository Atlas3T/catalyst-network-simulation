import numpy
import scipy
from propagation_time import getFilePathRoot
...
def percentage_under_cuttoff_latency(listofN,p,x,lats,ms):
    list = []
    for N in listofN:
        dist = propagation_time.load_transaction_time_distributionBis(N,p,x,lats)
        d = dist.flatten()
        no_under = numpy.count_nonzero(d < ms)
        result = no_under/len(d)
        list.append ([N,result])
    fileName = getProfileSetTimePath(ms,p,x,lats)
