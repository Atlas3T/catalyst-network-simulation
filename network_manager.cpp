...
void network_manager::process_send_event(t_t event_time, std::vector<unsigned char> data, nid_t from_node_id, std::vector<nid_t> to_node_ids){
    for(auto to_node_id : to_node_ids){
        ...
        sched.schedule_event(new recv_data_event(arrival_time, data, from_node_id, to_node_id, nodes));
    }
}
