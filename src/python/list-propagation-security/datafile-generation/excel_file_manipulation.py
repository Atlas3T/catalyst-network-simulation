    ...
    for sheet in get_sheet_names:
        try:
            ...
            grouped_data = data.groupby(['Total Producers','Correct Producers Ratio', 'Collected Updates Ratio', 
                                'Collected Votes Ratio', 'Collected Final Votes Ratio'],as_index =False)['Total Correct Ln(prod)',
                                                                                        'runs','Total Correct Ln(vote)',
                                                                                        'Runs With All Ln(prod)',
                                                                                        'Runs With All Ln(vote)',
                                                                                        'Runs With > 50% Correct','Runs With = Cn'].sum()

            grouped_data['num_correct_producers_Ln_prod'] = grouped_data['Total Correct Ln(prod)']/grouped_data['runs']
            grouped_data['num_correct_producers_Ln_vote'] = grouped_data['Total Correct Ln(vote)']/grouped_data['runs']
