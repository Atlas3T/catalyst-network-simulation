...
def get_result_output(num_of_producers, prop_correct_producers, prop_collected_vote, runs, results):
    avg_prod = numpy.sum(results[:, 0])
    avg_vote = numpy.sum(results[:, 1])
    num_list100_prod = (numpy.count_nonzero(results[:, 0] * prop_correct_producers)) 
    num_list100_vote = (numpy.count_nonzero(results[:, 1] == num_of_producers))
